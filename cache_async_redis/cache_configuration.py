...
import cachetools
from cache.test_cache import Strategy
...
#
# from cache.sync import fwk_cache as sync_fwk_cache
# from cache.async_ import fwk_cache as async_fwk_cache

# def cached(alias: str, key: str, namespace: Optional[str] = None,
#            ttl: Optional[int] = None):
#
#     def manage_params(func):
#         @wraps(func)
#         async def handled_async_func(*args, **kwargs):
#             fwk_cache: TestCache = FwkAioCache(alias)
#             # key = _key_from_args(func, args, kwargs)
#             value: SerializableData = fwk_cache.get(key)
#
#             if not value:
#                 value: SerializableData = func(*args, **kwargs)
#                 _: bool = fwk_cache.put(key, value)  # , namespace, ttl)
#                 # value: SerializableData = fwk_cache.get(key)
#             return value
#
#         @wraps(func)
#         def handled_sync_func(*args, **kwargs):
#             fwk_cache: TestCache = FwkAioCache(alias)
#             # key = _key_from_args(func, args, kwargs)
#             value: SerializableData = fwk_cache.get(key)
#             print("Decorator SYNC ", value)
#
#             if not value:
#                 value: SerializableData = func(*args, **kwargs)
#                 _: bool = fwk_cache.put(key, value)  # , namespace, ttl)
